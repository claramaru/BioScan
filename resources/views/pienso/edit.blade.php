...
<main class="main-wrap">
    <div class="top-bar">
        <div>
            <div class="page-title">Editar pienso</div>
            <div class="page-subtitle-feed">Actualiza un tipo de pienso del catalogo.</div>
        </div>
    ...
    <form method="POST" action="{{ route('pienso.update', $pienso->id_pienso) }}">
        @csrf
        @method('PUT')
        <div class="form-grid">
            <div>
                <div class="form-section">
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" name="nombre" id="nombre" class="form-control-custom {{ $errors->has('nombre') ? 'is-invalid' : '' }}" value="{{ old('nombre', $pienso->nombre) }}" required>
                            @error('nombre')<div class="error-msg">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-12">
                            <label class="form-label-custom d-flex align-items-center gap-2">
                                <input type="checkbox" name="activo" value="1" {{ old('activo', $pienso->activo) ? 'checked' : '' }}>
                                Activo
                            </label>
                        </div>
                    </div>
